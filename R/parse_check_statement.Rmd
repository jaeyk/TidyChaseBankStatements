...
df$Date <- as.Date(glue("2020/{df$Date}"))
...
```{r eval=FALSE}
knitr::purl(input = here("R", "parse_check_statement.Rmd"), 
            output = here("R", "parse_check_statement.r"))
```

# Test the function 

```{r}
source(here("R", "parse_check_statement.r"))

parse_check_statement(
    file_path = here("data", "check_statement.pdf"),
    current_year = 2020                   
        )
